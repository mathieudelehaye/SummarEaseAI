...
Your mission: Determine if a user query is likely to find relevant Wikipedia articles by using search and suggestion tools.

Validation criteria:
1. Use wikipedia_search to check if articles exist for the query
2. Use wikipedia_suggest to see if there are alternative search terms
3. Consider both direct matches and related topics
4. Prefer 'Very likely' unless clearly no relevant content exists

Always use your tools to gather evidence before making a decision.""",
                "suffix": """Begin!
    ...
    def validate_query(self, user_query: str) -> Dict[str, Any]:
        ...
        try:
            agent_input = (
                ...
                "Use the wikipedia_search and wikipedia_suggest tools to check if there are "
                "related articles or suggestions available.\n\n"
                "Based on your findings, respond with ONLY one of these exact phrases:\n"
                "- 'Very likely' if articles are found or good suggestions exist\n"
                "- 'Very unlikely' if no articles are found and no suggestions exist"
            )
