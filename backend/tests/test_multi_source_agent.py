...
class TestMultiSourceAgent:
    ...
    def test_plan_search_strategy_with_openai(
        ...
        strategy = agent.plan_search_strategy(
    ...
    def test_plan_search_strategy_rate_limited(
        ...
        assert (
            len(strategy["secondary_queries"]) == 0
        )  # Changed from > 0 to == 0 for MINIMAL mode
    ...
    def test_generate_fallback_search_plan_science(
        ...
        assert "related" in plan["secondary_queries"]
        assert any(
            "explanation" in query.lower() or "principles" in query.lower()
    ...
    def test_analyze_intent_fallback(
        ...
        )  # Changed from 'keyword_fallback' to 'fallback'
...
class TestMultiSourceAgentIntegration:
    ...
    def test_error_handling_in_planning(
        ...
        mock_query_gen = Mock()
        mock_query_gen_class.return_value = mock_query_gen
        mock_query_gen.generate_comprehensive_search_plan.side_effect = Exception(
            "OpenAI API error"
        )
