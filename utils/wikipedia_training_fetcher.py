...
class WikipediaPortalFetcher:
    ...
    def __init__(self):
        ...
        wikipedia.set_user_agent(
            "SummarEaseAI-Training/1.0 (https://github.com/your-repo)"
        )
            ...
            for subcategory in subcategories:
                ...
                try:
                    ...
                    search_results = wikipedia.search(
                        search_query, results=max_pages // len(subcategories)
                    )
                ...
                for page_title in pages[: max_articles // len(subcategories)]:
                    ...
                    if page_data:
                        self.existing_data = pd.concat(
                            [self.existing_data, pd.DataFrame([page_data])],
                            ignore_index=True,
                        )
    ...
    def get_categories_to_fetch(self) -> Dict[str, bool]:
        ...
        for category in self.portals.keys():
            ...
            if (
                current_samples >= min_samples * 1.5
            ):  # 50% more than minimum is our threshold
                should_fetch[category] = False
                logger.info(
                    f"Skipping {category} - already has {current_samples} samples (min: {min_samples})"
                )
            else:
                should_fetch[category] = True
                if current_samples > 0:
                    logger.info(
                        f"Will fetch {category} - has {current_samples} samples (min: {min_samples})"
                    )
                else:
    ...
    def check_category_balance(self) -> Dict[str, int]:
        ...
        if imbalanced_categories:
            ...
            for category, count, status in imbalanced_categories:
                if status == "over":
                    logger.info(
                        f"- Consider skipping {category} (has {count} samples, significantly more than average)"
                    )
                else:
                    logger.info(
                        f"- Prioritize collecting {category} (only {count} samples, significantly less than average)"
                    )
            ...
            if category_counts:
                ...
                for category, count in category_counts.items():
                    if count > min_count * 1.5:  # 50% more samples than minimum
                        ...
                        logger.warning(
                            f"Consider skipping {category} to maintain balance"
                        )
                        response = input(
                            f"Do you want to collect more {category} samples? (y/n): "
                        )
                        if response.lower() != "y":
            ...
            for portal_name, portal_info in self.portals.items():
                ...
                if category_counts:
                    ...
                    if current_count > min_count:
                        adjusted_max = min(
                            max_articles_per_category, min_count - current_count
                        )
                        if adjusted_max <= 0:
                            logger.info(
                                f"Skipping {portal_name} - already has sufficient samples"
                            )
                            continue
                        logger.info(
                            f"Adjusted target: {adjusted_max} articles to maintain balance"
                        )
    ...
    def fetch_page_training_data(
        self, page_title: str, intent_label: str
    ) -> Optional[Dict]:
    ...
    def collect_portal_training_data(
        self, portal_name: str, max_pages: int = 50
    ) -> List[Dict]:
        ...
        try:
            ...
            for page_title in pages:
                try:
                    # Get page data
                    page_data = self.fetch_page_training_data(
                        page_title, portal_info["intent_label"]
                    )
                    if page_data:
