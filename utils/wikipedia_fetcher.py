...
def preprocess_historical_query(query: str) -> tuple[str, bool]:
    ...
    for pattern, replacement in historical_events.items():
        if re.search(pattern, query_lower):
            logger.info(
                f"Historical query pattern detected: '{query}' -> '{replacement}'"
            )
...
def sanitize_wikipedia_content(content: str) -> str:
    ...
    if "{" in content or "}" in content:
        logger.info(
            f"🔧 Sanitized Wikipedia content: removed {content.count('{')} opening and {content.count('}')} closing curly braces"
        )
...
def fetch_article(topic: str) -> Optional[str]:
    ...
    try:
        ...
        if page.exists():
            ...
            logger.info(
                f"🔗 Article URL: https://en.wikipedia.org/wiki/{processed_topic.replace(' ', '_')}"
            )
            ...
            logger.info(
                f"Direct page not found for '{processed_topic}', trying search..."
            )
    ...
    try:
        ...
        for result in search_results:
            try:
                ...
                logger.info(
                    f"🔗 Search result URL: https://en.wikipedia.org/wiki/{result.replace(' ', '_')}"
                )
        ...
        if categories:
            ...
            for cat in categories:
                if any(
                    ...
                    for keyword in ["history", "science", "biography", "technology"]
                ):
...
def fetch_article_with_conversion_info(topic: str) -> tuple[Optional[str], str, bool]:
    ...
    try:
        ...
        article_content = (
            fetch_article(processed_topic) if was_converted else fetch_article(topic)
        )
...
def enhance_query_with_intent(query: str, intent: str, confidence: float) -> str:
    ...
    if confidence < 0.4:
        logger.info(
            f"🚫 Not enhancing query - confidence {confidence:.3f} below threshold 0.4"
        )
        return query
    ...
    if intent in intent_enhancements:
        ...
        if intent == "Science":
            ...
            if any(term in query_lower for term in specific_science_terms):
                logger.info(
                    f"🧪 Science query already contains specific terms - not enhancing: '{query}'"
                )
                return query
        ...
        if not has_intent_keywords:
            ...
            logger.info(
                f"✨ Enhanced query with intent '{intent}': '{query}' -> '{enhanced_query}'"
            )
            return enhanced_query
        else:
            logger.info(
                f"✅ Query already contains {intent.lower()} keywords - not enhancing: '{query}'"
            )
...
def search_and_fetch_article_info(
    ...
    try:
        ...
        logger.info(
            f"🌐 Wikipedia search API call: wikipedia.search('{processed_query}', results={max_results + 2})"
        )
        ...
        for result in search_results:
            try:
                ...
                logger.info(
                    f"📋 Available options: {e.options[:5]}..."
                )  # Show first 5 options
                try:
                    logger.info(
                        f"🔄 Trying first disambiguation option: '{e.options[0]}'"
                    )
                    logger.info(
                        f"🌐 Wikipedia page API call: wikipedia.page('{e.options[0]}')"
                    )
                    ...
                    logger.error(
                        f"❌ Failed to resolve disambiguation: {str(disambiguation_error)}"
                    )
                    continue
...
def search_and_fetch_article_agentic_simple(
    ...
    try:
        ...
        logger.info(
            f"🌐 Wikipedia search API call: wikipedia.search('{processed_query}', results={max_results})"
        )
        ...
        try:
            ...
            for alt_page in alternative_pages:
                try:
                    ...
                    logger.info(
                        f"🌐 Wikipedia page API call: wikipedia.page('{alt_page}')"
                    )
                    page = wikipedia.page(alt_page, auto_suggest=False)

                    logger.info("✅ Successfully fetched alternative page!")
                    logger.info(f"📄 Page title: {page.title}")
                    logger.info(f"🔗 Page URL: {page.url}")
                    logger.info("=" * 80)
                    ...
                    logger.warning(
                        f"⚠️ Alternative page '{alt_page}' also failed: {str(alt_error)}"
                    )
                    continue
            ...
            if len(search_results) > 1:
                try:
                    ...
                    logger.info(
                        f"🔄 Trying fallback to second result: '{fallback_page}'"
                    )
                    logger.info(
                        f"🌐 Wikipedia page API call: wikipedia.page('{fallback_page}')"
                    )
                    page = wikipedia.page(fallback_page, auto_suggest=False)

                    logger.info("✅ Successfully fetched fallback page!")
                    logger.info(f"📄 Page title: {page.title}")
                    logger.info(f"🔗 Page URL: {page.url}")
                    logger.info("=" * 80)
            ...
            logger.error(
                f"❌ General error fetching page '{selected_page}': {str(general_error)}"
            )
            return None
...
def simple_query_optimization(query: str) -> str:
    ...
    for pattern, replacement in optimizations.items():
        if (
            ...
            and "who was" not in pattern
            and "who were" not in pattern
        ):
