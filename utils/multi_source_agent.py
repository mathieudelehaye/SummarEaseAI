...
from backend.summarizer import summarize_article_with_intent, get_openai_api_key, LANGCHAIN_AVAILABLE
...
class MultiSourceAgent:
    ...
    def __init__(self, cost_mode: str = "BALANCED"):
        # Initialize BERT with the same model path as API
        self.bert_classifier = get_bert_classifier(str(BERT_MODEL_PATH))
        self.bert_model_loaded = self.bert_classifier.load_model() if self.bert_classifier else False
        ...
        logger.info(f"🎛️ Multi-Source Agent initialized in {self.cost_mode} mode")
...
"""
    ...
    def _analyze_intent(self, query: str) -> Dict[str, Any]:
        ...
        if any(word in query for word in ['who', 'person', 'born', 'died', 'biography']):
            intent = 'Biography'
        elif any(word in query for word in ['when', 'history', 'historical', 'century', 'era', 'period']):
            intent = 'History'
        elif any(word in query for word in ['how', 'science', 'scientific', 'technology', 'technical']):
            intent = 'Science'
        elif any(word in query for word in ['music', 'song', 'band', 'artist', 'album']):
            intent = 'Music'
        elif any(word in query for word in ['sport', 'game', 'player', 'team', 'championship']):
            intent = 'Sports'
        elif any(word in query for word in ['finance', 'money', 'economic', 'market', 'business']):
            intent = 'Finance'
        else:
            intent = 'General'
